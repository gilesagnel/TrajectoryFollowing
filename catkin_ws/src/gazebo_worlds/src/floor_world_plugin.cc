    ...
    class WorldGazeboPlugin : public WorldPlugin {
      ...
      public:
        void Load(physics::WorldPtr _world, sdf::ElementPtr _sdf) {
            physics::ModelPtr floorModel = _world->ModelByName("floor");
            floorModel->GetByName("visual_1");
            // rendering::VisualPtr floorVisual = boost::dynamic_pointer_cast<rendering::VisualPtr>(floorModel->GetByName("visual_1"));
            // printf("Material name -> %s\n", floorVisual->GetMaterialName()) 
            
            // floorVisual->SetMaterial("ServiceSim/WoodFloor"); 

            // // Iterate through the visuals in the link and find the right one
            // for (auto const &visual : floorLink->GetVisuals()) {
            //     if (visual->GetName() == "visual") {
            //         visual->SetMaterial("ServiceSim/WoodFloor"); 
            //         break; // Assuming you only have one visual named "visual"
            //     }
            // }

...
class BaseModel(nn.Module):
    def __init__(self, image_dim, state_dim):
        ...
        self.cnn = torch.nn.Sequential(
            torch.nn.Conv2d(image_dim[-1], 3, 11, stride=3, padding=1),
            torch.nn.ReLU(),
            torch.nn.MaxPool2d(5, stride=2),
            torch.nn.Conv2d(3, 6, 3),
            torch.nn.ReLU(),
            torch.nn.MaxPool2d(5, stride=2),
            torch.nn.Flatten())
        self.linear = torch.nn.Sequential(
            torch.nn.Linear(state_dim, 15 * 15 *6),
            torch.nn.ReLU()
        )
        

    def forward(self, image, env_state):
        out = self.cnn(image)
        out += self.linear(env_state)
        return out
...
class TD3(object):
    def __init__(self, state_dim, action_dim, max_action):
        ...
        self.base_model = BaseModel((224, 224, 1), state_dim)
        state_dim = 15 * 15 * 6
    ...
    def get_action(self, state):
        ...
        state = self.base_model(state[0], state[1])
    ...
    def train(
        ...
        for it in range(iterations):
            ...
            ns = self.base_model(next_state[0], next_state[1])
            ...
            s = self.base_model(state[0], state[1])
            current_Q1, current_Q2 = self.critic(s, action)
...
batch_size = 5 # 40  # Size of the mini-batch
...
while timestep < max_timesteps:
    ...
    action[0] = action[0].clip(0, max_action)
    action[1] = action[1].clip(-max_action, max_action)
